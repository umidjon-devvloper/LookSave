    ...
    <article className="card p-4 sm:p-5">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-3">
        ...
        <div className="flex items-center gap-2">
          {timer ? (
            <span
              className={`text-xs font-semibold tabular-nums ${timer.urgent ? 'text-danger' : 'text-muted-foreground'}`}
            >
              {timer.text}
            </span>
          ) : null}
      ...
      <div className="grid gap-4 py-4 sm:grid-cols-2">
        ...
        <div className="space-y-1.5 text-sm">
          ...
          {order.address?.text ? (
            <>
              <p className="text-foreground">{order.address.text}</p>
      ...
      {order.note ? (
        <p className="rounded-xl bg-surface2 px-3 py-2 text-sm italic text-muted-foreground">
          “{order.note}”
        </p>
      ) : null}
      ...
      <footer className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {order.status === 'new' ? (
            <Button variant="outline" type="button" disabled={busy} onClick={props.onSeen}>
              Ko'rdim
            </Button>
          ) : null}
          ...
          {(order.status === 'new' || order.status === 'seen') && (
            <>
              <Button variant="destructive" type="button" disabled={busy} onClick={props.onReject}>
                Rad etish
              </Button>
              <Button
                type="button"
          ...
          {order.status === 'confirmed' ? (
            <Button type="button" disabled={busy} onClick={props.onReady}>
              Tayyor
            </Button>
          ) : null}
          ...
          {order.status === 'ready' ? (
            <>
              <Button variant="outline" type="button" disabled={busy} onClick={props.onNoShow}>
                Mijoz kelmadi
              </Button>
              <Button
                type="button"
